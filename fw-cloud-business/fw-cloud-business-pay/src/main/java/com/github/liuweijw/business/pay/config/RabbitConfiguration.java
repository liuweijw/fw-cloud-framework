...
public class RabbitConfiguration {
	...
	/**
     * 发送到该队列的通知会在一段时间后过期进入到pay_notify_delay_quenu
     * 每个通知可以控制自己的失效时间
     */
    ...
    Queue delayQueuePerMessageTTL() {
        return QueueBuilder.durable(PAY_NOTIFY_DELAY_PRE_MESSAGE_TTL)
                           .withArgument("x-dead-letter-exchange", PAY_NOTIFY_DELAY_EXCHANGE) // DLX，dead letter发送到的exchange
    ...
    Queue delayQueuePerQueueTTL() {
        return QueueBuilder.durable(PAY_NOTIFY_DELAY_QUENU_ALL_TTL)
                           .withArgument("x-dead-letter-exchange", PAY_NOTIFY_DELAY_EXCHANGE) // DLX
                           .withArgument("x-dead-letter-routing-key", PAY_NOTIFY_DELAY_QUENU_NAME) // dead letter携带的routing key
    ...
    Queue delayProcessQueue() {
        return QueueBuilder.durable(PAY_NOTIFY_DELAY_QUENU_NAME)
                           .build();
    }
    ...
    Binding dlxBinding(Queue delayProcessQueue, DirectExchange delayExchange) {
        return BindingBuilder.bind(delayProcessQueue)
                             .to(delayExchange)
                             .with(PAY_NOTIFY_DELAY_QUENU_NAME);
    }
    ...
    Binding queueTTLBinding(Queue delayQueuePerQueueTTL, DirectExchange perQueueTTLExchange) {
        return BindingBuilder.bind(delayQueuePerQueueTTL)
                             ...
                             .with(PAY_NOTIFY_DELAY_QUENU_ALL_TTL);
    }
