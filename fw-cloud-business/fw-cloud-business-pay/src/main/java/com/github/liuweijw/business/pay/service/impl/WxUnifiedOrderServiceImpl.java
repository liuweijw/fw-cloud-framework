...
public class WxUnifiedOrderServiceImpl implements WxUnifiedOrderService {
	...
	@Override
	public R<Map<String, Object>> doWxUnifiedOrderRequest(String tradeType,
			PayOrder payOrder, Map<String, String> params) {
		
		try{
            String logPrefix = "【微信支付统一下单】";
            if(null == payOrder || null == params 
            		    || StringHelper.isBlank(tradeType)
            ...
            try {
                ...
                switch (tradeType) {
                    ...
                    case PayConstant.WxConstant.TRADE_TYPE_APP : {
                        ...
                        // 此map用于客户端与微信服务器交互
                        payInfo.put("sign", SignUtils.createSign(configMap,null, wxPayConfig.getMchKey(),false));
                    ...
                    case PayConstant.WxConstant.TRADE_TYPE_JSPAI : {
                        ...
                        payInfo.put("paySign", SignUtils.createSign(payInfo,null, wxPayConfig.getMchKey(), false));
                        map.put("payParams", payInfo);
                        break;
                    }
